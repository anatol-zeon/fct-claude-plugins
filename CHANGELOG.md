...
- `external_plugins/telegram/src/` — чистая логика вынесена в модули (`text`, `permissions`, `sanitize`, `transcript`, `mentions`, `access`) с `bun:test`-покрытием (83 теста: state machine `gate()`/access, chunking, transcript-парсинг, mention-детект, permission-regex, санитайзеры). `tsconfig.json` (strict, bundler resolution) + скрипты `test` / `test:unit` / `test:e2e` / `typecheck`.
- `external_plugins/telegram/test/e2e/` — e2e на реальный bridge: `Bun.spawn`'ит `bun server.ts` как subprocess, мокает `api.telegram.org` через `Bun.serve` (с `getMe` / long-poll `getUpdates` / `sendMessage` / `setMessageReaction` / `setMyCommands` / `sendChatAction`), цепляет минимальный line-delimited-JSON-RPC MCP-клиент к stdio. 4 сценария: unpaired-DM → паринг-код (и NO MCP-уведомления); allowlisted DM → `notifications/claude/channel` с meta; permission-relay round-trip (request → DM → `y <id>` → `permission` notification + ✅-реакция, без relay'я reply'я); `/status` DM. Описание — `test/e2e/README.md`. `server.ts` получил `TELEGRAM_API_ROOT` env-seam (передаётся в grammy как `client.apiRoot`; не задан = боевой `api.telegram.org`). Полный прогон: 87 тестов за ~1.8 s.
